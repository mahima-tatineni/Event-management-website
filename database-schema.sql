-- MLRIT Campus Event Hub Database Schema
...
-- Admin policies (using direct role check to avoid recursion)
CREATE POLICY "Admin can view all data" ON users FOR SELECT USING (
    (SELECT role FROM users WHERE id = auth.uid()) = 'admin'
);

CREATE POLICY "Admin can view all students" ON students FOR SELECT USING (
    (SELECT role FROM users WHERE id = auth.uid()) = 'admin'
);

CREATE POLICY "Admin can view all clubs" ON clubs FOR SELECT USING (
    (SELECT role FROM users WHERE id = auth.uid()) = 'admin'
);

CREATE POLICY "Admin can view all registrations" ON registrations FOR SELECT USING (
    (SELECT role FROM users WHERE id = auth.uid()) = 'admin'
);
...
CREATE POLICY "Admin can view all queries" ON queries FOR SELECT USING (
    (SELECT role FROM users WHERE id = auth.uid()) = 'admin'
);
...
CREATE POLICY "Admin can view platform settings" ON platform_settings FOR SELECT USING (
    (SELECT role FROM users WHERE id = auth.uid()) = 'admin'
);

CREATE POLICY "Admin can update platform settings" ON platform_settings FOR UPDATE USING (
    (SELECT role FROM users WHERE id = auth.uid()) = 'admin'
); 